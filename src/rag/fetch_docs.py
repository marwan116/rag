...
class DocumentFetcher(BaseModel):
    ...
    file_names_to_exclude: list[str] = Field(
    ...
    async def _fetch_docs(self, loader):
        ...
        blobs_and_paths_to_keep = [
            ...
            if all(
                ...
                file_name_to_exclude not in Path(blob_and_path[1]).name  # blob_and_path[1] is the path
                for file_name_to_exclude in self.file_names_to_exclude
            )
        ]
...
if __name__ == "__main__":
    ...
    if docs_path.exists():
        ...
        documents = asyncio.run(loader.run())
